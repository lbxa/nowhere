...
// import { appRouter } from "@nowhere/trpc";
import { createContext } from "@nowhere/trpc";
...
app.get("/", (req, res) => {
  res.json({
    ...
    availableEndpoints: {
      ...
      "POST /trpc/location.submit":
        "Submit user location (requires X-Device-ID header)",
    },
    trpcUsage: {
...
httpServer.listen(PORT, () => {
  ...
  console.log(
    `🌐 CORS Origin(s): ${Array.isArray(CORS_ORIGIN) ? CORS_ORIGIN.join(", ") : CORS_ORIGIN}`
  );
  ...
  if (NODE_ENV === "development") {
    console.log(`\n📋 Quick test commands:`);
    console.log(
      `   Health check: curl "http://localhost:${PORT}/trpc/health.check"`
    );
    console.log(
      `   Get stats: curl "http://localhost:${PORT}/trpc/health.stats"`
    );
    console.log(
      `   Get locations: curl "http://localhost:${PORT}/trpc/location.getAll"`
    );
